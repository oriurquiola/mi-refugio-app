...
- **No medir un elemento rotado por su `getBoundingClientRect()`.** El marcador del anillo mide 14×14, pero vive dentro de un contenedor que rota: su caja **alineada a los ejes** crece hasta ~19,8 px y solo vale 14 en los múltiplos de 90°. Buscarlo por `width === 14` falla casi siempre y parece que la animación está rota. Medir la **matriz de transformación del contenedor** y sacar el ángulo con `Math.atan2(m[2], m[1])`.

## G10 · `AnimatePresence initial={false}` congela las animaciones de todo lo que quede adentro
- **Síntoma (real, 2026-08-02):** al envolver el ejercicio de respiración en un `AnimatePresence` para el fundido hacia "¡Muy bien!", dejaron de funcionar **tres** animaciones a la vez: el marcador del anillo quedó quieto, el pulso de fin de ciclo no apareció y el círculo dejó de respirar (fijo en 115 px = `222 × 0.52`, su último keyframe).
- **Causa:** `initial={false}` no afecta solo al hijo directo: viaja por el `PresenceContext` y **cancela la animación de entrada de todos los `motion` descendientes**. Cualquier animación que viva de `initial` → `animate` salta directo al último keyframe y se queda ahí.
- **Chequeo:** si algo "se muestra pero no se mueve", mirar si quedó bajo un `AnimatePresence initial={false}`. Un círculo detenido se ve idéntico a uno pausado, así que en un screenshot no salta a la vista: hay que **medir** (ver G9).
- **Solución:** poner `initial={false}` solo cuando adentro no haya `motion` que dependa de su entrada. Si el bloque envuelto no define `initial`/`animate` (solo `exit`), el flag no aporta nada y se puede quitar sin efectos: es el caso de `BreathingExercise.tsx`, donde está comentado en el código.
